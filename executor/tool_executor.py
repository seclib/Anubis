...
def create_default_tools() -> dict[str, ToolFunction]:
    """Build the default tool registry without importing the agent layer."""
    return {
        "read_file": read_file,
...
class ToolExecutor:
    ...
    def __init__(self, tools: Mapping[str, ToolFunction]) -> None:
...
TOOLS: dict[str, ToolFunction] = create_default_tools()
DEFAULT_EXECUTOR = ToolExecutor(TOOLS)


def refresh_dynamic_tools() -> dict[str, ToolFunction]:
    """Load generated tools from disk into the executable registry."""
    dynamic_tools = DEFAULT_EXECUTOR.refresh_dynamic_tools()
    TOOLS.update(DEFAULT_EXECUTOR.tools)
    return dynamic_tools


_DYNAMIC_TOOLS_LOADED: bool = False


def _ensure_dynamic_tools_loaded() -> None:
    """Charge les outils dynamiques une seule fois au démarrage."""
    global _DYNAMIC_TOOLS_LOADED
    if not _DYNAMIC_TOOLS_LOADED:
        refresh_dynamic_tools()
        _DYNAMIC_TOOLS_LOADED = True
...
def execute_tool(tool: str, args: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    _ensure_dynamic_tools_loaded()
    return DEFAULT_EXECUTOR.execute(tool, args)
