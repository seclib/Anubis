...
def read_input() -> str | None:
    ...
    try:
        line = input(f"\033[1;32m🜏 anubis [{datetime.now().strftime('%H:%M')}]\033[0m \033[1;30m>\033[0m ").strip()
    except EOFError:
        return None
    except KeyboardInterrupt:
        console.print()
...
def _banner_body() -> str:
    ...
    left = ANUBIS_SIGIL.splitlines()
    right = [
        f"[bold]Anubis CLI v{VERSION}[/bold]",
        "[dim]Hermes-grade local agent[/dim]",
        "",
        "[bold]Active Model[/bold]",
        f"[cyan]{OLLAMA_MODEL}[/cyan]",
        "",
        "[bold]Available Tools[/bold]",
        f"[dim]{_tool_preview()}[/dim]",
        "",
        "[bold]Available Skills[/bold]",
        f"[dim]{', '.join(skills)}[/dim]",
        "",
        f"[dim]{len(TOOLS)} tools · max {MAX_STEPS} steps · /help for commands[/dim]",
        f"[dim]endpoint: {OLLAMA_BASE_URL}[/dim]",
        f"[dim]project: {_compact_path(PROJECT_ROOT)}[/dim]",
    ]
    height = max(len(left), len(right))
    left.extend([""] * (height - len(left)))
    right.extend([""] * (height - len(right)))
    left_width = max(len(line) for line in left)
    return "\n".join(
        f"[bright_black]{escape(left_line):<{left_width}}[/bright_black]   {right_line}"
...
def print_banner():
    console.clear()
    console.print(Text(ANUBIS_ASCII, style="bold white"), justify="center")
    console.print(
        Panel(
            _banner_body(),
            title="[bold white] ANUBIS-AGENT [/bold white]",
            subtitle="[dim]Ahoy. Type yer message or /help for orders.[/dim]",
            border_style="bright_black",
            box=box.SQUARE,
            padding=(1, 2),
        )
    )
    console.print("[dim]Examples:[/dim] [green]read config.py[/green]  [dim]·[/dim] [green]fix tests[/green]  [dim]·[/dim] [green]/run refactor memory[/green]\n")
