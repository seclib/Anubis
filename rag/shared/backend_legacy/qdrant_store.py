...
class _Settings:
    ...
    qdrant_collection: str = os.getenv("QDRANT_COLLECTION", "anubis_chunks")


settings = _Settings()
from rag.shared.backend_legacy.chunker import Chunk
...
class QdrantStore:
    def __init__(self) -> None:
        self.client = QdrantClient(url=settings.qdrant_url) if QdrantClient else None
        self.embedder = LocalEmbedder()
        self.collection = settings.qdrant_collection

    def ensure_collection(self) -> None:
        if not self.client:
            raise RuntimeError("qdrant client package unavailable")
        try:
