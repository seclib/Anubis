...
APP_HOME="${APP_HOME:-/app/anubis-agent}"
...
if [ "$HERMES_MEMORY_BACKEND" = "qdrant" ]; then
  ...
  python - "$QDRANT_URL" "$QDRANT_RETRY_ATTEMPTS" "$QDRANT_RETRY_DELAY" <<'PY'
import json
import sys
import time
import urllib.error
...
delay = max(0.1, float(sys.argv[3]))
    ...
    try:
        with urllib.request.urlopen(url, timeout=5) as response:
            ...
            if response.status == 200 and "result" in payload:
                ...
                print(
                    f"[anubis-entrypoint] External Qdrant is reachable; "
...
print(
    f"[anubis-entrypoint] ERROR: Cannot connect to Qdrant at {url}. "
    "Set QDRANT_URL to a reachable Docker service URL or localhost URL.",
    file=sys.stderr,
)
raise SystemExit(1)
PY
