...
def save_memory(memory: dict[str, Any]) -> None:
    """Save memory to disk with atomic write and list size capping."""
    ...
    # Tronquer les listes pour éviter une croissance infinie du fichier
    for key in ("steps", "actions", "tool_results"):
        if len(memory.get(key, [])) > 500:
            memory[key] = memory[key][-500:]
    if len(memory.get("errors", [])) > 100:
        memory["errors"] = memory["errors"][-100:]
    if len(memory.get("agent_messages", [])) > 200:
        memory["agent_messages"] = memory["agent_messages"][-200:]

    # Écriture atomique : évite la corruption en cas d'interruption
    path = MEMORY_FILE
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp_path = path.with_suffix(".tmp")
    tmp_path.write_text(json.dumps(memory, indent=2, ensure_ascii=False, default=str))
