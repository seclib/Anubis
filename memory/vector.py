...
def load_vector_store() -> dict[str, Any]:
    ...
    if not isinstance(store, dict):
        store = _empty_store()
    if not isinstance(store.get("documents"), list):
        store["documents"] = []
    store.setdefault("version", 1)
    store.setdefault("embedding_model", EMBEDDING_MODEL)
    store.setdefault("created_at", _now_iso())
    store["updated_at"] = store.get("updated_at") or _now_iso()
...
def save_vector_store(store: dict[str, Any]) -> None:
    ...
    tmp.write_text(json.dumps(store, indent=2, ensure_ascii=False, default=str))
    os.replace(tmp, path)
    with _store_cache_lock:
        _store_cache = store
...
def index_agent_history(memory: dict[str, Any]) -> dict[str, Any]:
    """Indexe uniquement le dernier message d'agent (incrémental)."""
    messages = memory.get("agent_messages", [])
    if not isinstance(messages, list) or not messages:
        return {"status": "skipped", "agent_history_documents": 0}

    last_message = messages[-1]
    if not isinstance(last_message, dict):
        return {"status": "skipped", "agent_history_documents": 0}

    text = json.dumps(last_message, ensure_ascii=False, default=str)
    content_hash = _content_hash(text)
    index = len(messages) - 1
    doc_id = f"agent_history:{index}:{content_hash[:12]}"
    ...
    documents.append({
        "id": doc_id,
        "kind": "agent_history",
        "source": str(last_message.get("agent", "unknown_agent")),
        "chunk_index": index,
        "content_hash": content_hash,
        "text": text,
